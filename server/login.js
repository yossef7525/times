...
addrow: async function (req, res) {
  ...
  sql.query(`select email from users where email='${req.body.email}'`, async function (error, results, fields) {
    if (error) throw error;
   if(results.length <1){
  var str = `INSERT INTO users (id, firstname, lastname, image, email) VALUES (${results.length+1}, "${req.body.firstName}","${req.body.lastName}","${req.body.photoUrl}" ,"${req.body.email}")`
       console.log(str);
  sql.query(str,async function (error, results, fields) {
   if (error) throw error;
   res.send(results)
